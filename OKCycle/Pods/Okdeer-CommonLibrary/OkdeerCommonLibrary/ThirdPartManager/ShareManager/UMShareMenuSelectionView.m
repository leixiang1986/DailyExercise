...
#define UMSocial_BgGray_View_Alpha 0.4
...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        #warning  判断系统的版本
        CGFloat UMSocial_Item_Width = 60 + (/* DISABLES CODE */ (10) ?  5 : 0 );   // 默认的宽度
        if ([UIScreen mainScreen].bounds.size.width > [UIScreen mainScreen].bounds.size.height) {
            UMSocial_Item_Width = [UIScreen mainScreen].bounds.size.height/320 * UMSocial_Item_Width;
        }else{
            UMSocial_Item_Width = [UIScreen mainScreen].bounds.size.width/320 * UMSocial_Item_Width;
        }
        self.itemSize = CGSizeMake(UMSocial_Item_Width, 67);
...
//        [self creatCancelButton];
        ...
        //获取可分享平台
        [self.sharePlatformInfoArray removeAllObjects];
        self.sharePlatformInfoArray = [[NSMutableArray alloc] initWithArray:[self obtainSharePlatformTypeArray]];
        if (self.sharePlatformInfoArray.count == 0) {
            return nil;
        }
...
- (void)setCustomPlatformTypeArray:(NSArray *)customPlatformTypeArray{
    _customPlatformTypeArray = customPlatformTypeArray;
    [self.sharePlatformInfoArray removeAllObjects];
    self.sharePlatformInfoArray = [[NSMutableArray alloc] initWithArray:[self obtainSharePlatformTypeArray]];
    NSLog(@"sharePlatformInfoArray ===== %@",self.sharePlatformInfoArray);
}

/**
 * 获取分享的平台
 */
- (NSArray *)obtainSharePlatformTypeArray{
    //获取可分享平台
    NSMutableArray *platformArray = [[NSMutableArray alloc] init];
    
    if (self.customPlatformTypeArray.count == 0) {
        self.customPlatformTypeArray =  [UMSocialManager defaultManager].platformTypeArray;
    }
    
    for (NSNumber *platformType in self.customPlatformTypeArray) {
...
#warning 设置颜色
   // [cancelButton setTitleColor:UIColorFromHex(COLOR_999999) forState:UIControlStateNormal];
    cancelButton.backgroundColor = [UIColor whiteColor];
    cancelButton.layer.cornerRadius = UMSocial_Menu_CornerRadius;
    CALayer *buttonLayer = [CALayer layer];
    buttonLayer.frame = CGRectMake(0, 0, CGRectGetWidth(cancelButton.frame), 1);
//    buttonLayer.backgroundColor = UIColorFromHex(COLOR_E2E2E2).CGColor;
    [cancelButton.layer addSublayer:buttonLayer];
...
- (void)resetSelfFrame
{
    ...
    //设置偏移
    selfFrame.origin.y = superView.frame.size.height - selfFrame.size.height - self.cancelButton.frame.size.height - UMSocial_MenuAndCancel_Space - UMSocial_Menu_Bottom_Space;
...
- (void)show
{
    ...
    [UIView animateWithDuration:0.2 animations:^{
        CGRect frame = self.frame;
        frame.origin.y = self.superview.frame.size.height - frame.size.height - self.cancelButton.frame.size.height - UMSocial_MenuAndCancel_Space - UMSocial_Menu_Bottom_Space  ;
        self.frame = frame;
