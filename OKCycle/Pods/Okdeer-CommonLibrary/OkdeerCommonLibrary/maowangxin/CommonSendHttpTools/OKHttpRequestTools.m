                                ...
                                failure:(OKHttpFailureBlock)failureBlock
{
    ...
    void (^failResultBlock)(NSError *) = ^(NSError *error){
        ...
        /******添加begin by lx******/
        if (error.code != -1) { //如果不是因为重复请求而失败，就标记为该请求已经结束。否则还是还是保持正在请求的状态
    ...
    /******添加begin by lx******/
    if (requestModel.isRequesting) {   //已经在请求了,不再请求
        if (failResultBlock) {
            failResultBlock([NSError errorWithDomain:@"重复请求" code:[kRepeatRequest integerValue] userInfo:nil]);
        }
        return nil;
    }
    else {  //没有请求就标记为正在请求
