...
+ (UIButton *)buttonTitle:(NSString *)title target:(id)target action:(SEL)action
{
    ...
    CGFloat width = [self calculateheight:button.titleLabel.font withString:title].width + 8;
    button.frame = CGRectMake(0, 0, width + 16, 30);
    if (title.length == 0 ) {
        button.backgroundColor = [UIColor clearColor];
    }else{
        button.backgroundColor = UIColorFromHex(0x0099ff);
    }
 ...
 *  根据字体大小 计算
 */
+ (CGSize)calculateheight:(UIFont *)font withString:(NSString *)string
{
    ...
    if ([UIDevice currentDevice].systemVersion.floatValue > 7.0) {
        contSize = [string sizeWithAttributes:@{NSFontAttributeName:font}];
    }else{
        contSize = [string sizeWithFont:font];
    }
    return contSize;
}
 ...
 *
 */
+ (UIButton *)buttonMustItemWithImage:(NSString *)image highImage:(NSString *)highImage target:(id)target action:(SEL)action
{
    UIButton *button = [UIButton buttonWithType:UIButtonTypeCustom];
 ...
 *
 */
+ (instancetype)buttonItemWithImage:(NSString *)image highImage:(NSString *)highImage size:(CGSize)size target:(id)target action:(SEL)action
{
    UIButton *button = [UIButton buttonWithType:UIButtonTypeCustom];
