...
// 注释,影响编译 by chenzl, extension引用view, view中的ActionSheet引用了extension
...
/**
 ...
 @return 返回当前请求的对象
 */
+ (NSURLSessionDataTask *)sendExtensionOKRequest:(OKHttpRequestModel *)requestModel
    ...
    sessionDataTask = [OKHttpRequestTools sendOKRequest:requestModel success:^(id returnValue) {
        ...
        if (!requestModel.attemptRequestWhenFail) {
            ...
            if (countNum<3) {
                ...
                sessionDataTask = [OKHttpRequestTools sendExtensionOKRequest:requestModel success:successBlock failure:failureBlock];
